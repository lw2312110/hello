...
def read_page(url, page_num, keyword):  # 模仿浏览器post需求信息，并读取返回后的页面信息
    ...
    if page_num == 1:
        ...
        boo = 'false'
    page_data = urllib.urlencode({'first':boo,'pn':page_num,'kd':keyword})  # 通过页面分析，发现浏览器提交的FormData包括以下参数
...
def read_tag(page, tag):
    page_json1 = json.loads(page)
    page_json = page_json1['content']['result']  # 通过分析获取的json信息可知，招聘信息包含在返回的result当中，其中包含了许多其他参数
...
if __name__ == '__main__':
    ...
    file_name = raw_input('抓取完成，输入文件名保存：')
